...
public class PessoaServiceImpl implements PessoaService {
    ...
    public Page<PessoaDTO> findAllWithEagerRelationships(Pageable pageable) {
        return pessoaRepository.findAllWithEagerRelationships(pageable).map(pessoaMapper::toDto);
    }

    @Override
    @Transactional(readOnly = true)
    public Optional<PessoaDTO> findOne(UUID id) {
        log.debug("Request to get Pessoa : {}", id);
        return pessoaRepository.findOneWithEagerRelationships(id).map(pessoaMapper::toDto);
    }
