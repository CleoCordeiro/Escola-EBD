...
class EnderecoResourceIT {
    ...
    @Test
    @Transactional
    void getAllEnderecosByResponsaveisIsEqualToSomething() throws Exception {
        ...
        em.flush();
        endereco.addResponsaveis(responsaveis);
        enderecoRepository.saveAndFlush(endereco);
        UUID responsaveisId = responsaveis.getId();

        // Get all the enderecoList where responsaveis equals to responsaveisId
        defaultEnderecoShouldBeFound("responsaveisId.equals=" + responsaveisId);

        // Get all the enderecoList where responsaveis equals to UUID.randomUUID()
        defaultEnderecoShouldNotBeFound("responsaveisId.equals=" + UUID.randomUUID());
    }

    @Test
    @Transactional
    void getAllEnderecosByPessoasIsEqualToSomething() throws Exception {
        Pessoa pessoas;
        if (TestUtil.findAll(em, Pessoa.class).isEmpty()) {
            enderecoRepository.saveAndFlush(endereco);
            pessoas = PessoaResourceIT.createEntity(em);
        } else {
            pessoas = TestUtil.findAll(em, Pessoa.class).get(0);
        }
        em.persist(pessoas);
        em.flush();
        endereco.addPessoas(pessoas);
        enderecoRepository.saveAndFlush(endereco);
        UUID pessoasId = pessoas.getId();

        // Get all the enderecoList where pessoas equals to pessoasId
        defaultEnderecoShouldBeFound("pessoasId.equals=" + pessoasId);

        // Get all the enderecoList where pessoas equals to UUID.randomUUID()
        defaultEnderecoShouldNotBeFound("pessoasId.equals=" + UUID.randomUUID());
    }
